...
//build markup for table row and header
	var markup = "<tr><th></th>" //add th for checkbox background-color: #4CAF50;
	$.each(inputs, function(index, value ){
...
function formBuilder (appendTo, appendedId, addButtonLabel, removeButtonLabel, inputs, nullvalue){
	...
	$(appendTo)
	.append('<table id="' + appendedId + '" class="data-records">' + headerMarkup)
	.append("<p></p><button class='addItem button' type='button'>" + addButtonLabel +"</button><button class='removeItem button' type='button'>" + removeButtonLabel +"</button>")
	
	//add an initial row
	$('table#' + appendedId).append(rowMarkup)
